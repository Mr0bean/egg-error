...
class EggError {
   ...
   * @param {Object} msg           ctx 返回错误信息
   * @param {function} after         事后处理
   * @param {string} logName       业务报错服务tag 如 router|order
   */
  ...
  /**
   * 错误信息处理
   * @param {Object} ctx   透明填充context
   * @param {Object} fill  字段填充
   */
  outputError(ctx, fill) {
    ...
    return {
      afterResult, throw: () => {
        ctx.throw(this.statusCode || 500, msg);
      },
    };
  }
